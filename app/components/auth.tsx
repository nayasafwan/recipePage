...
import { setUsername } from "../redux/userSlider";
    ...
    <ModalComponent>
      <div className=" relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-1/3 w-full sm:h-auto">
        ...
        <div className="bg-white px-4 pb-4 pt-5 sm:p-7 sm:pb-4">
          {isLogin ? (
            <LoginModal toggleAuth={toggleAuth} />
          ) : (
            <SignupModal toggleAuth={toggleAuth} />
          )}
        </div>
      </div>
    </ModalComponent>
...
const LoginModal = ({ toggleAuth }: { toggleAuth: () => void }) => {
  ...
  const loginHandler = async () => {
    ...
    try {
      ...
      let responseData = response.data.data.login;
      ...
      toast.error(
        (err as any)?.response?.data.data.login.message || "An error occurred"
      );
    }
  };
    ...
    <>
        ...
        <div>
          ...
          <input
            ...
            className={`border rounded w-full py-2 px-3 ${
              messageErrors.usernameError
                ? "border-red-500"
                : "border-slate-300"
            } border-slate-300 text-gray-700 bg-white focus:border-gray-700 focus:ring-gray-700 focus:border-1.75 focus:outline-none`}
            type="text"
            placeholder="Email or Username"
          />
          <p className="text-red-500 text-sm ml-0.5">
            {messageErrors.usernameError}
          </p>
        </div>
        <div>
          <div className="flex justify-between items-center">
            ...
            <p className="text-primary text-base cursor-pointer hover:text-orange-400">
              Forgot password?
            </p>
          </div>
          ...
          <div className="relative">
            <input
              ...
              className={`border rounded w-full py-2 px-3 ${
                messageErrors.passwordError
                  ? "border-red-500"
                  : "border-slate-300"
              } text-gray-700 bg-white focus:border-gray-700 focus:ring-gray-700 focus:border-1.75 focus:outline-none`}
      ...
      <div className=" mt-10 mb-5 py-3">
        ...
        <p className="mt-10 text-center text-lg font-medium">
          ...
          <span
            onClick={toggleAuth}
            className="text-primary cursor-pointer hover:text-orange-400"
          >
            Signup now
          </span>
        </p>
...
const SignupModal = ({ toggleAuth }: { toggleAuth: () => void }) => {
  ...
  const signupHandler = async () => {
    ...
    try {
      ...
      let responseData = response.data.data.signup;
  ...
  return (
    <React.Fragment>
      ...
      <h3 className="font-bold text-3xl mb-3 mt-2 text-center">
        Create an Account
      </h3>
      <form className="mt-10 space-y-4">
        ...
        <div>
          ...
          <input
            ...
            id="signup-email"
            className={`border rounded w-full py-2 px-3 ${
              messageErrors.emailError ? "border-red-500" : "border-slate-300"
            } text-gray-700 bg-white focus:border-gray-700 focus:ring-gray-700 focus:border-1.75 focus:outline-none`}
            type="text"
            placeholder="Email"
          />
          <p className="text-red-500 text-sm ml-0.5">
            {messageErrors.emailError}
          </p>
        </div>
        <div>
          <h2 className="text-lg font-medium mb-1">Password</h2>
        ...
        <div>
          ...
          <div className="relative">
            <input
              ...
              className={`border rounded w-full py-2 px-3 ${
                messageErrors.confirmPasswordError
                  ? "border-red-500"
                  : "border-slate-300"
              } text-gray-700 bg-white focus:border-gray-700 focus:ring-gray-700 focus:border-1.75 focus:outline-none`}
